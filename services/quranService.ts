...
export const fetchSurahVerses = async (surahId: number, page: number = 1): Promise<Verse[]> => {
  const response = await fetch(
    `${QURAN_API_BASE}/verses/by_chapter/${surahId}?language=en&words=false&translations=${TRANSLATION_ID}&fields=text_uthmani&per_page=50&page=${page}`
  );
  const data = await handleResponse(response);
  return data.verses;
};
...
export const fetchTafseer = async (verseKey: string, tafseerId: number): Promise<Tafseer> => {
  // Correct V4 endpoint for Tafsir is /quran/tafsirs/{tafsir_id}?verse_key={verse_key}
...
export const searchVerses = async (query: string): Promise<Verse[]> => {
  // Correct V4 endpoint parameter is 'query' instead of 'q'
